...
def verify_doi(doi):
    ...
    try:
        ...
        print(f"   Network/parse error for DOI {doi}: {e}")
        return None
...
def main():
    ...
    for c in citations:
        ...
        if real is None:
            print(f"  COULD NOT VERIFY — DOI does not resolve on Crossref.")
            all_passed = False
            print()
            continue
        ...
        if title_score > 0.75 and journal_score > 0.6 and year_match:
            print(f"  MATCH (title {title_score:.0%}, journal {journal_score:.0%})")
        else:
            print(f"  MISMATCH (title {title_score:.0%}, journal {journal_score:.0%}, year: {year_match})")
            all_passed = False
        print()
